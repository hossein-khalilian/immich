<script lang="ts">
  import FolderSharedLink from '$lib/components/folder-page/folder-shared-link.svelte';
  ...
  interface Props {
    ...
    onClose: (result?: { action: 'sharedLink' } | { action: 'sharedUsers'; data: FolderUserAddDto[] }) => void;
  }
  ...
  onMount(async () => {
    // @ts-expect-error - getAllSharedLinks will support folderId when SDK is regenerated
    sharedLinks = await getAllSharedLinks({ folderId: folder.id });
    const data = await searchUsers();

    // remove folder owner
    users = data.filter((user) => user.id !== folder.ownerId);

    // Remove the existed shared users from the folder
    for (const sharedUser of folder.folderUsers) {
      ...
      {#if sharedLinks.length > 0}
        ...
        <Stack gap={4}>
          {#each sharedLinks as sharedLink (sharedLink.id)}
            <FolderSharedLink {folder} {sharedLink} />
          {/each}
        </Stack>
      {/if}
