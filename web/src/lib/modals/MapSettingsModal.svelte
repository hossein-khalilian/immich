<script lang="ts">
  ...
  import { lang, type MapSettings } from '$lib/stores/preferences.store';
    ...
    if (currentLang === 'fa') {
      if (convertedSettings.dateAfter && convertedSettings.dateAfter.includes('/')) {
        try {
          ...
          console.error('Failed to convert Jalali dateAfter to Gregorian', error);
        }
      }
      ...
      if (convertedSettings.dateBefore && convertedSettings.dateBefore.includes('/')) {
        try {
          ...
          console.error('Failed to convert Jalali dateBefore to Gregorian', error);
        }
      }
    }
