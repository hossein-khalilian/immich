...
describe('RecentAlbums component', () => {
  it('sorts albums by most recently updated', async () => {
    ...
    expect(links[0]).toHaveAttribute('href', `/library/albums/${albums[2].id}`);
    expect(links[1]).toHaveAttribute('href', `/library/albums/${albums[1].id}`);
    expect(links[2]).toHaveAttribute('href', `/library/albums/${albums[3].id}`);
  });
});
